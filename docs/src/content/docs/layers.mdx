...
## Use another layer type

Use <Api of="Layer" /> for a layer type without a built-in composable. You can
wrap it in a typed composable to reuse in your app or publish in a plugin library:

<Code code={region(layers, "plugin-layer")} lang="kotlin" title="PluginMeshLayer.kt" />

The renderer must support the layer type and its properties. See the
<Api of="LayerProperties" /> API reference for the available property setters.

...
declare sources and layers in the map state's style composition. A source holds the
...
<Code code={inside("val map = rememberMapState {", region(layers, "amtrak-1"))} lang="kotlin" title="App.kt" />

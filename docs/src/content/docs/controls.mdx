...
and <Api of="LocalCameraPadding" /> exposes its camera padding. These values remain
available inside nested composables and layouts. A nested map provides its own context.
...
The built-in presets use the larger of `cameraPadding` and safe-drawing insets on
...
`cameraPadding` also shifts the camera center. For custom controls that follow this
padding, read <Api of="LocalCameraPadding" /> and apply it to their container.

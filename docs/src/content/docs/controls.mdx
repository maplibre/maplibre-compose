...
1. The scale bar reads the scale from map state while it draws. A camera
   animation then redraws the bar each frame without recomposing the overlay.
   Passing a `Double` works too, and recomposes the bar on every change.

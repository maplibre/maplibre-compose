...
`SymbolLayer` draws labels in the map. `MapAnchors` draws Compose UI on the
map: a chip, a button, or a callout.

`Modifier.anchor` places each child on a geographic position, the way
`Modifier.align` places a child in a box. Draw `MapAnchors` as a sibling of
...
<Code code={region(controls, "anchors")} lang="kotlin" title="App.kt" />

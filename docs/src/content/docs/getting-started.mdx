...
Both Android runtimes package `armeabi-v7a`, `arm64-v8a`, and `x86_64`.
Android requires API 24 or later. OpenGL requires OpenGL ES 3.0; Vulkan requires
a Vulkan-capable device and driver. Packaging an ABI does not guarantee that its
device supports the selected backend. There is no automatic backend fallback.

If another dependency includes native libraries for additional ABIs, restrict
your application's `ndk.abiFilters` to the supported ABIs above. FFI does not
publish 32-bit x86 libraries.

ARMv7 compile/link and APK coverage are checked in CI. ARMv7 rendering and
lifecycle have not yet been verified on a compatible device.

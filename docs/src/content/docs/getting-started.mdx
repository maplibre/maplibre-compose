...
Use the Kotlin/JS or Kotlin/Wasm browser target. Compile the Kotlin/JS target to
ES modules. Non-ESM builds are not supported.
...
Call `installMapLibreCompose` before creating a map to configure its worker URL.
Compose provides the shared WebGL render target.

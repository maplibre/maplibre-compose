...
iOS app target's **Other Linker Flags**:
...
On desktop, a `ComposeMapHost` supplies the window's graphics context. For Java
AWT windows, use `rememberAwtComposeMapHost`. For an alternative Compose host,
implement `ComposeMapHost` yourself.
...
When you run your app, you see the default [demotiles] map. To get a detailed
map with the features you expect, proceed to

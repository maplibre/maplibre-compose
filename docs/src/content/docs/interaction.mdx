...
Code that recognizes gestures itself can pass them through <Api of="MapState.panBy" />, <Api of="MapState.scaleBy" />,
<Api of="MapState.fling" />, and <Api of="MapState.click" />. They follow the
camera permissions and callbacks in `MapInteractions` and report
<Api of="CameraMoveReason.GESTURE" />, like a pointer gesture. They take logical
pixels and do nothing while no map is presented. See
[Present on an Android Surface](/maplibre-compose/android-surface/).

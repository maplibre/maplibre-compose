...
The providers and `LocationState` live in the `org.maplibre.compose:location`
artifact, which `maplibre-compose` includes. An application that tracks
location without a map depends on that artifact directly.

...
draws the latest reading, and <Api of="LocationTrackingEffect" /> keeps the camera
...
`locationState.lastReading` is null until the first reading arrives, so the puck
initially draws nothing. Afterward, the state retains the last reading when

...
Google Play services is available, and the framework providers otherwise. When
several Android backends are available, the defaults use the highest-priority
backend, and the first backend id in alphabetical order if those priorities
match. Use `rememberFusedLocationProvider` and
`rememberFusedOrientationProvider` to configure the fused providers directly.

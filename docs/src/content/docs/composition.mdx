...
A handle's identity, such as a layer's `id`, `type`, `source`, and `sourceLayer`,
is a plain value. A read that asks the engine for a value, such as a layer
property or a feature's state, is a `suspend` function: on MapLibre Native the
value comes from the map's own thread, so call it from a coroutine rather than
from composition. A write returns at once and applies on that thread. When the
engine rejects a written value, the library logs a warning and the previous
value stays in place. The same applies to `mapState.style.transition`, `light`,
`sky`, and `projection`.

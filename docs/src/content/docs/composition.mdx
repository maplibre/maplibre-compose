...
A handle write returns before the map applies it. When the engine rejects a
written value, the library logs a warning and the previous value stays in
place. The same applies to `mapState.style.transition`, `light`, `sky`, and
`projection`.

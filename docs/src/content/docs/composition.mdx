...
<Api of="MaplibreMap" /> presents that state in Compose UI. Android hosts can also
[present it on a caller-owned Surface](/maplibre-compose/android-surface/). `MapState` provides camera,
...
A handle write does not wait for the map to apply it. On MapLibre Native the
write runs on the map's own thread after the call returns; MapLibre GL JS
applies it during the call. When the engine rejects a written value, the
library logs a warning and the previous value stays in place. The same applies
to `mapState.style.transition`, `light`, `sky`, and `projection`.

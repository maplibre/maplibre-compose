...
composable function and call that function from each style block. A function
with no receiver works in `rememberMapState`, `MapRuntime.createMapState`, and
`MapRuntime.createSnapshotter`. A function with a <Api of="MapStyleScope" />
receiver takes the `mapState` value from the scope, so it applies only to an
interactive map.

...
evaluates the style composition against it. Your sources and layers are
declared again, so they persist across the switch without extra code. An
`Anchor` that names a base layer applies only when that layer exists in the new
style.

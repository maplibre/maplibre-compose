...
- Writing Kotlin JS bindings for
  [MapLibre GL JS](https://maplibre.org/maplibre-gl-js/docs/).
- Using those bindings to implement browser support in the `jsMain` module of
  MapLibre Compose.

Next steps:

- Add support for for programmatic layer styling (sources, layers, expressions,
  images, etc).
...
The goal is to overhaul the documentation to make it easier for folks not
already familiar with MapLibre to use the library, and to make LLMs more
reliable at writing correct correct code using MapLibre Compose.
...
This project would also benefit from some work to improve the experience of
developing MapLibre Compose for desktop. The biggest pain points right now are:

- Building MapLibre Native core from source in local dev and every CI run.
- Regressions due to limited automatic tests on all platforms.
- Brittle local development environment.
...
but are not being worked on today. But if you're interested and would like to
...
- Explore to what extend JS bindings to Kotlin can be shared between Kotlin JS
  and Kotlin WASM.
...
gestures for those mobile platforms, so the focus here is on providing a rich
experience for desktop and web users.
...
- Explore the available inputs APIs on macOS, Linux (X11 and Wayland), Windows,
  and web browsers.
...
JVM+Native wrapper. Today, we wrap the MapLibre Native core for desktop, but go
through the MapLibre Native Android Java/Kotlin bindings on Android and MapLibre
...
Compose UI, so this may just involve writing bare KMP wrappers for MapLibre
Native on some platforms, or rendering map snapshots, or integrating with some
...
The goal is to perform all map rendering within Compose, instead of including
...
I'm not sure if this is possible, but if it is, it'll likely be done by
performing map rendering to a texture, and using a Compose Multiplatform runtime
shader to render that texture.

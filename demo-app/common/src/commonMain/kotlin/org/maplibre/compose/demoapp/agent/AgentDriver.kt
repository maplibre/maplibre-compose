...
internal class AgentDriver(
  ...
  fun camera() = state.mapState.toDto()

  suspend fun jumpCamera(request: CameraUpdateRequest): CameraDto {
    request.validateRanges()
    markDemoMapReload()
    state.mapState.setCamera(state.mapState.camera.merged(request))
    return camera()
  }
  ...
  suspend fun animateCamera(request: CameraUpdateRequest): CameraDto {
    ...
    markDemoMapReload()
    state.mapState.animateCamera(
      state.mapState.camera.merged(request),
  ...
  suspend fun featuresJson(xPx: Float, yPx: Float, layerIds: Set<String>?): String {
    ...
    val features = state.mapState.queryRenderedFeatures(offset = offset, layerIds = layerIds)
    return FeatureCollection(features).toJson()
  }

  /** Pans the map as if dragged by ([request]'s dx, dy) pixels, via the camera. */
  suspend fun pan(request: PanRequest): CameraDto {
    requireFinite("dxPx", request.dxPx)
    requireFinite("dyPx", request.dyPx)
    val size =
      state.mapState.viewport?.size ?: throw AgentException(503, "the map has no viewport yet")
    ...
    val atCenter = state.mapState.positionFromScreenLocation(center) ?: return camera()
    val atMoved = state.mapState.positionFromScreenLocation(moved) ?: return camera()
    val current = state.mapState.camera
   ...
   */
  suspend fun zoom(request: ZoomRequest): CameraDto {
    ...
    val anchor =
      if (request.x != null && request.y != null) {
        with(density) { DpOffset(request.x.toDp(), request.y.toDp()) }
          .let(state.mapState::positionFromScreenLocation)
      } else {
        null
      }
    ...
    state.mapState.setCamera(current.copy(target = target, zoom = newZoom))
    return camera()
  }
  ...
  private fun MapState.toDto(): CameraDto {
    val position = camera
    return CameraDto(
      position =
        CameraPositionDto(

...
// Keep this scope internal until camera commands share the control API described in
// https://github.com/maplibre/maplibre-compose/issues/1305.
/**
 ...
 */
internal suspend fun MapState.withCameraInput(block: suspend CameraInputScope.() -> Unit) {
...
internal class CameraInputScope(
  ...
  fun scaleBy(scale: Double, anchor: DpOffset? = null) {
    validateScale(scale)
    validateAnchor(anchor)
    target.inputScaleBy(scale, anchor, gestureToken = token)
  }

  fun rotateAndPitchBy(bearingDelta: Double, pitchDelta: Double, anchor: DpOffset? = null) {
  ...
  suspend fun panByAwaitingTransition(deltaX: Double, deltaY: Double, duration: Duration) {
    validate(deltaX, deltaY)

...
class BrowserStyleStateTest {
  ...
  fun a_source_reports_the_attribution_its_tilejson_carries(): Promise<*> = runBrowserMapTest {
    ...
    try {
      ...
      setBrowserMapContent {
        val mapState = rememberMapState(baseStyle = tileJsonStyle)
        state = mapState
        MaplibreMap(
          state = mapState,
          modifier = Modifier,
  ...
  fun switching_to_a_tilejson_style_keeps_the_attribution(): Promise<*> = runBrowserMapTest {
    ...
    try {
      ...
      var loads = 0
      setBrowserMapContent {
        val mapState = rememberMapState(baseStyle = current)
        state = mapState
        MaplibreMap(
          state = mapState,
          modifier = Modifier,
  ...
  fun a_source_added_after_load_reports_late_tilejson_attribution(): Promise<*> =
    runBrowserMapTest {
      ...
      try {
        ...
        setBrowserMapContent {
          val mapState =
            rememberMapState(baseStyle = BaseStyle.Empty) { node = LocalStyleNode.current }
          state = mapState
          MaplibreMap(
            state = mapState,
            modifier = Modifier,

...
class BearingAccuracyRenderingTest {
  ...
  fun sector_shape_zoom_style_and_hit_testing_match_on_both_engines() = runMapTest {
    createMapFixture().use { fixture ->
      ...
      layer.paint("bearing", (const(0f).compile(context)).asLayerProperty())
      ...
      layer.paintTransition("bearing", TransitionOptions(Duration.ZERO))
      layer.paintTransition("bearing-accuracy", TransitionOptions(Duration.ZERO))
      layer.paintTransition("bearing-accuracy-radius", TransitionOptions(Duration.ZERO))
      layer.paintTransition("bearing-accuracy-color", TransitionOptions(Duration.ZERO))

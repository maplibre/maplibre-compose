 ...
 *
 * Keeping these platform-neutral lets every FFI host use the same touch feel while mouse and
 * keyboard affordances remain available on every host as well. Values expressed as Android
 * dimensions are converted to dp before they reach these helpers.
 */
internal object ClassicAndroidGestureMath {
  ...
  /** The exact Android pinch equation, returned as the multiplicative scale FFI expects. */
  ...
  fun scaleVelocity(
    ...
    val durationMillis =
      (ln(abs(zoomDelta) + 1.0 / Math.E.pow(2.0)) + 2.0) * VELOCITY_ANIMATION_DURATION_MULTIPLIER
  ...
  fun rotationVelocity(
    ...
    val durationMillis =
      (ln(abs(angularVelocity) + 1.0 / Math.E.pow(2.0)) + 2.0) *
        VELOCITY_ANIMATION_DURATION_MULTIPLIER

 ...
 */
@Immutable
internal class MlnFfiMapExtent
private constructor(
  ...
  override fun toString(): String =
    "MlnFfiMapExtent(logical=${width}x$height, physical=${physicalWidth}x$physicalHeight, " +
      "scale=$scaleFactor)"

  companion object {
    /** An extent with no renderable area. */
    val Empty: MlnFfiMapExtent = MlnFfiMapExtent(0, 0, 1.0, 0, 0)

    /**
     * Builds an extent from a logical (dp-equivalent) size and scale factor, deriving the physical
     * size.
     */
    fun fromLogical(width: Int, height: Int, scaleFactor: Double): MlnFfiMapExtent {
      val scale = normalizeScale(scaleFactor)
      if (width <= 0 || height <= 0) return Empty
      return MlnFfiMapExtent(
        width = width,
        height = height,
        scaleFactor = scale,

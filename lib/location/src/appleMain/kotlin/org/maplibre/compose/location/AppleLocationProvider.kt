...
internal constructor(private val requester: AppleLocationPermissionRequester) : LocationProvider {
  /** Creates a provider with its own permission requester. */
  public constructor() : this(AppleLocationPermissionRequester())
  ...
  override fun updates(request: LocationRequest): Flow<LocationEvent> =
    callbackFlow<AppleLocationCallback> {
      ...
      .map { callback ->
        when (callback) {
          is AppleLocationCallback.Update -> callback.event
          is AppleLocationCallback.Failure ->
            LocationEvent.Unavailable(callback.error.asUnavailableReason())
        }
      }

  private class Delegate(private val channel: SendChannel<AppleLocationCallback>) :
    ...
    override fun locationManager(manager: CLLocationManager, didFailWithError: NSError) {
      channel.trySend(AppleLocationCallback.Failure(didFailWithError))
    }
...
private sealed interface AppleLocationCallback {
  data class Update(val event: LocationEvent.Update) : AppleLocationCallback

  data class Failure(val error: NSError) : AppleLocationCallback
}

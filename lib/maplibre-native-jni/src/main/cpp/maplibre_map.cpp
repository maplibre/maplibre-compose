...
struct MapWrapper {
  ...
  MapWrapper(
    mbgl::Map *m, maplibre_jni::JniMapObserver *o,
    maplibre_jni::AwtCanvasRenderer *r
  )
      : map(m), observer(o), renderer(r) {}
};

MapWrapper *getWrapper(JNIEnv *env, jMapLibreMap map) {
  auto peer = java_classes::get<MapLibreMap_class>().getNativePeer(env, map);
  auto ptr = java_classes::get<AutoCleanPointer_class>().getRawPtr(env, peer);
...
jboolean JNICALL MapLibreMap_class::tick(JNIEnv *env, jMapLibreMap map) {
  try {
    auto *wrapper = getWrapper(env, map);
    return wrapper->renderer->tick();
  } catch (const std::exception &e) {
    smjni::java_exception::translate(env, e);
    return false;
  }
...
jlong JNICALL MapLibreMap_class::nativeInit(
  ...
  try {
    ...
    auto *wrapper =
      new MapWrapper(map.release(), observer.release(), renderer.release());

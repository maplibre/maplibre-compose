...
device-test manifest now registers `MapStateRecreationActivity`. This
registration restores the activity-recreation test after its ownership-API
rename.
...
| -------------- | -------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Retained       | `BrowserMapStyleStateTest`: detached map and viewport tests    | Web recreation, replay, and presentation readiness                           |
| Rewritten      | `BrowserMapStyleStateTest`: four source and style-switch tests | Public Web style state and TileJSON metadata                                 |
